...
exports.checkUndefinedCriteria = function(criteria, cb) {
  var k;
  if (criteria.where) {
    for(k in criteria.where) {
      if(criteria.where[k] === undefined) {
        throw new Error('Passing undefined in WHERE clauses can be dangerous >:(');
      }
    }
  } else {
    for(k in criteria) {
